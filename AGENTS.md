...
- 所有代码修改交付前必须运行 `uv run pyright/ruff check .`，直至通过验证（无报错）才可交付给用户。
...
## 提交与合并
- 提交信息保持“前缀(范围): 简述”的格式，简述部分必须使用中文，例如 `simulator: 添加循环赛统计`，突出修改模块。
- 使用常见英文类型前缀（如 `feat`, `fix`, `chore`, `docs`），整体形如 `fix(simulator): 添加循环赛统计`，方便统一过滤。
